...
class Exp:
    ...
    def _build_model(self):
        ...
        """
        window_size = 8
        height = (64 // window_size + 1) * window_size
        width = (64 // window_size + 1) * window_size
        self.model = SwinIR(upscale=1, img_size=(height, width),
                   window_size=window_size, img_range=1., depths=[6, 6, 6, 6],
                   embed_dim=48, num_heads=[6, 6, 6, 6], mlp_ratio=2, upsampler='pixelshuffledirect').to(self.device)
        self.model = ESR(num_in_ch=1, num_out_ch=4, num_feat=64, num_block=4, num_grow_ch=32).to(self.device)
        """
    ...
    def test(self, args):
        model_dict = "results/Debug/checkpoints/0.pth"
